...
%     signal(i,1:4) = vec;
    ...
    if y > 600
        ...
        if mod(i,100) == 0
            ...
            charVecC3(:) = [media(3), moda(3), mediana(3), desvEstandar(3), maximo(3), minimo(3), RMS(3), potencia(3), CAFc3, CAPc3, CAB(3), mediaER(3), modaER(3), medianaER(3), desvEstandarER(3), maximoER(3), minimoER(3), RMSER(3), potenciaER(3), CAFc3ER, CAPc3ER, CABER(3)];
            charVecC4(:) = [media(4), moda(4), mediana(4), desvEstandar(4), maximo(4), minimo(4), RMS(4), potencia(4), CAFc4, CAPc4, CAB(4), mediaER(4), modaER(4), medianaER(4), desvEstandarER(4), maximoER(4), minimoER(4), RMSER(4), potenciaER(4), CAFc4ER, CAPc4ER, CABER(4)];

            charVec = cat(2,charVecC3,charVecC4);
            charVec = rescale(charVec,-1,1,'InputMin',minCV,'InputMax',maxCV);

            prediccion_Aux = prediccion;
            prediccion = round(net.predict(charVec))
            
            if prediccion(1) == 1 && prediccion(1) == prediccion_Aux(1)
                write(bt,"2");
            elseif prediccion(2) == 1 && prediccion(2) == prediccion_Aux(2)
                write(bt,"1");
            else
                write(bt,"0");
            end
